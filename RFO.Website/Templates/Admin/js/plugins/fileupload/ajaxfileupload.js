...
jQuery.extend({
    ...
    createUploadForm: function (id, fileElementIds, data) {
        ...
        for (i = 0; i < fileElementIds.length; i++) {
            var fileElementId = fileElementIds[i];
            var fileId = 'jUploadFile' + fileElementId + id;
    ...
    uploadHttpData: function (r, type) {
        ...
        if (type == "json") {
            var preIndex = data.indexOf('<pre');
            if (preIndex != -1) {
                var closePreIndex = data.indexOf('>', preIndex);
                if (closePreIndex) {
                    data = data.substring(closePreIndex + 1, data.length - 6);
                }
            }
            eval("data = " + data);
        }
